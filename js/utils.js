...
function isValid(x, y) {
    return (x >= 0 && y >= 0 && x < row && y < col);//Checking if the co-ordinate is valid or not
}
...
function set(className, x = -1, y = -1) {
    //if any co-ordinate provided then update the source and taget with that co-ordinates
    if (isValid(x, y)) {
        matrix[x][y].classList.add(className);
    }
    //If not provided the randomly set co-ordinate of source and target 
    else {
